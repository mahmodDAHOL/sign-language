...
class DataManipulation:
    def __init__(self, percent_test, percent_val, project_path):
    ...
    def train_test_val_split_idxs(self, total_rows):
        ...
        print("Train-test-val split: %i training rows, %i test rows, %i validation rows" % (
            len(self.training_idxs), len(self.test_idxs), len(self.val_idxs)))
    ...
    def cluster_features(self, img_descs, cluster_model):
        ...
        print(
            f"descriptors before clustering {all_train_descriptors.shape[0]}")
        print(f"descriptors shape {all_train_descriptors.shape}")

        # Cluster descriptors to get codebook
        print(f"Using clustering model { repr(cluster_model)}...")
        print(
            f"Clustering on training set to get codebook of {n_clusters} words")
    ...
    def cnn_preprocess(project_path):
